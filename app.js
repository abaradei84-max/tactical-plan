...
function normalizeRow(r){
  const memberCosts={}; MEMBERS.forEach(m=>memberCosts[m]=n(readField(r,[norm(m)])));
  return {
    customer:String(readField(r,['cust name','customer','customer name'])||'Unknown').trim(),
    specialty:String(readField(r,['specialty'])||'Unknown').trim(),
    zone:String(readField(r,['zone'])||'Unknown').trim(),
    activity:String(readField(r,['type of activity','type','activity'])||'').trim(),
    cost:n(readField(r,['cost'])), memberCosts
  };
}
...
function effectiveCost(r){const active=selectedMembers();if(!active.length)return r.cost||0;return active.reduce((total,m)=>total+(r.memberCosts[m]||0),0)}
...
function availableValues(key){
  if(key==='member'){
    ...
    return MEMBERS.filter(m=>rows.some(r=>passes(r,'member')&&memberHasCost(r,m)));
  }
...
function initFilters(){
  ...
  FILTERS.forEach(([key,label])=>{
    ...
    box.innerHTML=`<label>${label}</label><input id="fs-${key}" class="filter-search" type="search" placeholder="Search ${label.toLowerCase()}…"><div id="f-${key}" class="filter-options"></div><div class="filter-actions"><button data-a="all" type="button">Select visible</button><button data-a="clear" type="button">Clear</button></div>`;
    root.appendChild(box);
    box.querySelector('input').addEventListener('input',e=>{searches[key]=norm(e.target.value);renderFilter(key)});
    box.querySelector('[data-a="all"]').addEventListener('click',()=>{const vals=[...box.querySelectorAll('.check[data-value]')].map(x=>x.dataset.value);selections[key]=new Set([...selections[key],...vals]);updateAll()});
    box.querySelector('[data-a="clear"]').addEventListener('click',()=>{selections[key]=new Set();updateAll()});
  });
  renderFilters();
}
function renderFilter(key){
  ...
  host.innerHTML=vals.length?vals.map(v=>`<label class="check" data-value="${esc(v)}"><input type="checkbox" ${selections[key].has(v)?'checked':''}><span>${esc(displayValue(key,v))}</span></label>`).join(''):'<div class="check">${rows.length?'No options':'Load Excel to populate'}</div>';
...
function renderChips(){
  const chips=[];FILTERS.forEach(([k,label])=>selections[k].forEach(v=>chips.push(`<span class="chip"><strong>${esc(label)}:</strong> ${esc(displayValue(k,v))}<button data-k="${k}" data-v="${esc(v)}" type="button">×</button></span>`)));
  const host=$('selectedChips');if(!host)return;host.innerHTML=chips.join('');
  host.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{selections[b.dataset.k].delete(b.dataset.v);updateAll()}));
}
function group(data,key,metric=effectiveCost){const m=new Map();data.forEach(r=>{const k=typeof key==='function'?key(r):r[key];const value=typeof metric==='function'?metric(r):(r[metric]||0);m.set(k,(m.get(k)||0)+value});return [...m.entries()]}
...
function updateDetail(data){
  const m=new Map();data.forEach(r=>{const k=[r.specialty,displayActivity(r.activity),r.zone].join('|||');if(!m.has(k))m.set(k,{specialty:r.specialty,activity:displayActivity(r.activity),zone:r.zone,customers:new Set(),cost:0});const x=m.get(k);x.customers.add(r.customer);x.cost+=effectiveCost(r)});
  const arr=[...m.values()].filter(x=>x.cost!==0).sort((a,b)=>b.cost-a.cost).slice(0,100);$('detailTable').innerHTML=arr.map(x=>`<tr><td>${esc(x.specialty)}</td><td>${esc(x.activity)}</td><td>${esc(x.zone)}</td><td>${x.customers.size}</td><td>${fmt(x.cost)}</td></tr>`).join('')||'<tr><td colspan="5">No matching data</td></tr>';
}
...
async function loadExcel(file){$('status').textContent='Reading Excel…';const buf=await file.arrayBuffer();const wb=XLSX.read(buf,{type:'array'});const ws=findTrackingSheet(wb);if(!ws)throw new Error('Tracking 2027 sheet not found');const raw=XLSX.utils.sheet_to_json(ws,{defval:'',raw:true});rows=raw.map(normalizeRow).filter(r=>r.customer!=='Unknown'||r.specialty!=='Unknown'||r.cost!==0||rowMembers(r).length);resetFilters();$('status').textContent=`${file.name} • ${rows.length.toLocaleString()} rows`;updateAll()}
